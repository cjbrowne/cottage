...
# don't always build tools_fat (remove the comment to build it)
# it's not needed, and can make the output from Make longer and harder to read
# when debugging
...
## test target to run qemu easily
test: $(BUILD_DIR)/main_floppy.img
	qemu-system-i386 -fda $(BUILD_DIR)/main_floppy.img

## debug target to run bochs easily
